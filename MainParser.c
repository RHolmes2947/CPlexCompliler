...
extern Plex_none startParser(Plex_none);
extern int startScanner(BufferPointer sc_buf);

static Plex_none printParserError(str fmt, ...);
static Plex_none displayParser(BufferPointer ptrBuffer);
static Plex_long getParserFileSize(str fname);
static Plex_none callGarbageCollector(Plex_none);
...
Plex_none printParserError(str fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(Plex_none)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
Plex_long getParserFileSize(str fname) {
	FILE* input;
	Plex_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
Plex_none displayParser(BufferPointer ptrBuffer) {
...
Plex_none callGarbageCollector(Plex_none) {
